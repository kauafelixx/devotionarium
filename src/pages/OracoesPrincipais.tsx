...
const OracoesPrincipais = () => {
  const category = getCategoryById("oracoes-principais");
  const [selectedPrayerId, setSelectedPrayerId] = useState(
    category?.prayers[0]?.id || ""
  );

  const selectedPrayer = category?.prayers.find(
    (p) => p.id === selectedPrayerId
  );
...
export default OracoesPrincipais;
